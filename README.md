...
контрольную сумму и всё настраивают. Установщик — это мастер: сам находит ноду,
образ серверов и фреймворк, проверяет ключи и Wings и в конце пишет, что осталось
сделать в панели. Повторный запуск той же команды — меню: обновить, перенастроить,
удалить. `update.sh` умеет откатываться сам, если
...
| выбран, установлен | версия фиксируется, ничего не качается | обновляется, если вышла версия **новее** стоящей |
...
### Swiftly: бета-версии и фиксация версии

После обновлений CS2 исправления у Swiftly выходят **сначала в бете**, а стабильная
версия падает вместе с сервером (так было с CS2 1.41.8.2: стабильная v1.4.10
не находила 21 сигнатуру, фикс вышел в v1.4.11-beta).

| Переменная | Что делает |
|---|---|
| `SWIFTLY_BETA=1` | ставить и бета-версии (pre-release на GitHub), берётся самая новая |
| `SWIFTLY_BETA=0` | только стабильные. Уже стоящая бета **не откатывается**: обновление будет, когда выйдет стабильная новее неё |
| `SWIFTLY_VERSION=v1.4.10` | ставить ровно эту версию — в том числе более старую, для отката. Работает и при выключенном автообновлении. Пусто — выбирать самим |

Хранить две копии Swiftly рядом не нужно: в его каталоге живут ваши плагины и
конфиги, а вернуться на любую версию — это `SWIFTLY_VERSION` и перезапуск.
Версии сравниваются по semver: `v1.4.11-beta.10` новее `beta.9` и `v1.4.10`,
но старше `v1.4.11`.

### gameinfo.gi после обновления CS2

У каждого сервера своя копия `game/csgo/gameinfo.gi`: в неё дописываются аддоны,
а `/mnt` только на чтение. Когда Valve меняет этот файл вместе с обновлением
игры, контейнер при запуске сам берёт новую версию с хоста, а прежнюю кладёт в
`game/csgo/gameinfo.gi.bak`. Ваши правки в файле сохраняются, пока Valve его
не меняла: для этого рядом хранится `gameinfo.gi.host` — версия, с которой
сделана копия.
