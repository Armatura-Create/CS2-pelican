...
> Объём запросов: одна команда на сервер в секунду, всем серверам одновременно —
> поэтому отсчёт идёт положенное время при любом числе серверов. При
> `UPDATE_COUNTDOWN_TIME=300` и пяти серверах это 1500 запросов к панели за цикл
> обновления. Уменьшайте `UPDATE_COUNTDOWN_TIME`, если панель к этому чувствительна.
>
> После запуска серверов updater ждёт до 3 минут, пока каждый дойдёт до `running`.
> Если сервер упал (например, аддон несовместим с новой версией CS2), в логе будет
> `Сервер … НЕ поднялся после обновления CS2` — смотрите его консоль в панели.
